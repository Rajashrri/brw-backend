...
// Get data from collection by table_name
const getTableData = async (req, res) => {
  try {
    ...
    const collection = mongoose.connection.collection(tableName); // dynamic collection access
    ...
    const getPageOptions2  = async (req,res)=>{
        try {
            const page = await Page.find();
            res.status(200).json({
                msg:page,
    ...
    const addtemplate = async(req,res)=>{
        try {
            ...
            res.status(200).json({msg:'Template update Successfully',template_id:template_id});  
        }catch(error){
    ...
    const getadditionalmodules = async (req, res) => {
     ...
     const updatetmeplateid = async(req,res)=>{
        const template_id = req.body.id2;
        const bussacc = req.body.id;
  ...
  try {
    ...
    const customer = await Template.findOneAndDelete({ _id: id });
    if (!customer) {
      return res.status(404).json({ msg: "Templete not found" });
    }
  const deletedata = await Templatefields.deleteMany(({template_id:id}));
    ...
    const inmodagclt =async (req,res)=>{
        try {
            const { pageid, pagename, id: bussacc } = req.body;
...
const getTemplateId = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      template_id: customer.admintemplate_id || null, // use admintemplate_id
    });
  } catch (error) {
...
const getroutesdata = async (req, res) => {
  try {
    ...
    if (!customer) {
      return res.status(200).json({ msg: [], info: "Customer not found" });
    }
    ...
    // Fetch template user data (if needed)
    ...
    if (tempfields.length > 0 || tempfieldsuser.length > 0) {
      ...
      pagesData = pages
        .filter(page => page.page_name && page._id && page.page_url)
        .map(page => ({
    ...
    return res.status(200).json({
      msg: pagesData,
      info: pagesData.length > 0 ? "Data found" : "No Template Data Found"
    });
    ...
    //     }
    //   };
      const getTableFieldData = async (req, res) => {
  try {
    const { tableName, fieldName } = req.params;

    if (!tableName || !fieldName) {
      return res.status(400).json({ error: "Table name and field name are required" });
    }

    const collection = mongoose.connection.collection(tableName);

    // Only project the specified field
    const data = await collection
      .find({}, { projection: { [fieldName]: 1 } })
      .toArray();

    // Optional: Remove _id if not needed
    const cleanedData = data.map(item => item[fieldName]).filter(Boolean);
...
module.exports = {getfieldbyurl,getPageOptions2,getTableFieldData, getclientmodules,getroutesdata,getTableData, inmodagclt, getpage1, updatetmeplateid, getadditionalmodules,getsearchtemplates, gettemfields, getpage, create_pages,update_pages, getdata, getall, deletedata , deletefield, addsidebaricon , getSidebarOptions,addtemplate,gettemplates,getPageOptions,getTemplateId,deletetemp};
