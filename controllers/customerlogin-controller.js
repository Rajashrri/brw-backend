...
const register = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Internal Server Error", details: error.message }); // Send the error message in the response
  }
};
  ...
  try {
    ...
    if (user) {
      ...
      // main().catch(console.error);
       ...
       res.status(200).json({
        type: true,
        msg: "Email and Password Verified Successfully",
        // token: await userExist.generateToken2(),
        email: email,
        custid: userExist._id.toString(),
      });
    } else {
      res.status(200).json({
        type: false,
      ...
      if (page == "forgot") {
        const result = await Customer.updateOne(
          { email },
          {
            $set: {
              otp: rand,
              forgototpcount: userExist.forgototpcount + 1,
            },
          },
          {
            new: true,
